...
function Player.novo(x, y, hud)
    ...
    player.x = x
    player.y = y
    player.id = "player" -- Utilizado na função de colisao local
    ...
    scriptTeclado.novo (player) -- ativando a função global .novo do script do teclado.
    ...
    local function verificarColisao (self, event)
        ...
        -- quando o id do outro corpo for "chao"
            ...
            elseif event.other.id == "moeda" then
                display.remove (event.other)
                hud.somar() -- executa a função do script HUD para somar os pontos.
            end
        end
    end 
